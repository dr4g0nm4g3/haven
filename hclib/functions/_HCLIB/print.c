...
/* _HCLIB_print( const char *, struct _HCLIB_status_t * )
...
/* Macro: DELIVER
   ...
   depending on the contents of 'status' (struct _HCLIB_status_t).
...
*/
static void int2base( intmax_t value, struct _HCLIB_status_t * status )
{
    ...
    */
    ...
    if ( status->flags & E_lower )
    {
        ...
        DELIVER( _HCLIB_digits[ digit ] );
    }
    else
    {
        /* Uppercase letters. Array only used here, only 0-F. */
        DELIVER( _HCLIB_Xdigits[ digit ] );
    }
    }
}
...
static int testprintf( char * buffer, size_t n, const char * format, ... )
{
    ...
    struct _HCLIB_status_t status = { 0, 0, n, 0, 0, buffer, 0, 0, NULL, NULL };
    memset( buffer, '\0', 100 );
    va_start( status.arg, format );
    if ( *(_HCLIB_print( format, &status )) != '\0' )
    {
        printf( "_HCLIB_print() did not return end-of-specifier on '%s'.\n", format );
        ++rc;
    }
    va_end( status.arg );
    return status.i;
