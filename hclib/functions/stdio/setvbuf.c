...
int setvbuf( struct _HCLIB_file_t * _HCLIB_restrict stream, char * _HCLIB_restrict buf, int mode, size_t size )
{
    ...
    */
    if ( ( ! stream->status & _HCLIB_VIRGINSTR ) ||
         ( ( mode != _IOFBF ) && ( mode != _IOLBF ) && ( mode != _IONBF ) ) ||
         ! ( stream->status & _HCLIB_LIBBUFFER ) )
    {
        return -1;
    }
    if ( mode == _IONBF )
    ...
    */
    ...
    stream->status |= mode;
    stream->bufsize = size;
    stream->status &= ~_HCLIB_VIRGINSTR;
    return 0;
#endif
}
